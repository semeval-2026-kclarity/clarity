#!/usr/bin/env python3
"""
Ensemble inference + Codabench packaging for CLARITY Task B (evasion level),
PLUS a mapped Task A (clarity level) file derived from the Task B outputs.
...
python scripts/submissions/make_evasion_submission.py \
  --csv data/raw/semeval_test/clarity_task_evaluation_dataset.csv \
  --models-root "../model_outputs/task_b/roberta_large_task_b_stratified_qa_marked" \
  --outdir "../model_outputs/task_b/roberta_large_task_b_stratified_qa_marked/submissions" \
  --batch-size 32 \
  --max-length 512

python scripts/submissions/make_evasion_submission.py \
  --csv data/raw/semeval_test/clarity_task_evaluation_dataset.csv \
  --models-root "../model_outputs/task_b/roberta_large_task_b_stratified_balanced_weighted_qa_marked" \
  --outdir "../model_outputs/task_b/roberta_large_task_b_stratified_balanced_weighted_qa_marked/submissions" \
  --batch-size 32 \
  --max-length 512
"""
...
# Task B label mapping (from task_b.yaml)
IDX2EVASION = {
    0: "Explicit",
    1: "Implicit",
    2: "Dodging",
...
def discover_best_models(models_root: Path) -> list[Path]:
    ...
    if not model_dirs:
        raise FileNotFoundError(
            f"No best_model dirs found under: {models_root} (expected seed_*/best_model)"
        )
    return model_dirs
...
def best_prefix_strip(state_keys: list[str], target_keys: set[str]) -> str:
    """
    Finds the best prefix to strip so checkpoint keys match model keys.
    """
...
def robust_load_model_task_b(model_dir: Path, device: torch.device) -> AutoModelForSequenceClassification:
    """
    Manual load of weights with prefix stripping.
    """
    logging.info(f"Loading model (manual weights): {model_dir}")

    config = AutoConfig.from_pretrained(model_dir, local_files_only=True)
    config.num_labels = 9
    config.id2label = {i: IDX2EVASION[i] for i in range(9)}
    config.label2id = {IDX2EVASION[i]: i for i in range(9)}
    ...
    logging.info(f"  Manual load - missing={len(missing2)} unexpected={len(unexpected)}")
    ...
    if bad_missing or bad_unexpected:
        ...
        raise RuntimeError(
            ...
            "Checkpoint might not be a full HF model state_dict, or wrong architecture."
        )
...
def predict_with_model(
    ...
    model = robust_load_model_task_b(model_dir, device)
...
def majority_vote(pred_matrix: np.ndarray, prob_tensor: np.ndarray) -> np.ndarray:
    """
    pred_matrix: [M, N] ints
    prob_tensor: [M, N, C] floats
    """
...
def main():
    ...
    ap.add_argument("--models-root", required=True, help="Root containing seed_*/best_model")
    ...
    for i, model_dir in enumerate(model_dirs, start=1):
        logging.info(f"[{i}/{len(model_dirs)}] Predicting with {model_dir.parent.name}/{model_dir.name} ...")
        ...
        dist = {IDX2EVASION[int(k)]: int(v) for k, v in zip(uniq, cnt)}
    ...
    final_evasion = [IDX2EVASION[int(i)] for i in final_ids]

    # ----------------------------
    # Write Task B submission
    # ----------------------------
    pred_path_b = out_task_b / "prediction"
    write_prediction_file(pred_path_b, final_evasion)
    logging.info(f"Wrote Task B prediction file: {pred_path_b} (lines={len(final_evasion)})")
    ...
    # ----------------------------
    # Map to Task A and write it
    # ----------------------------
    final_clarity = map_evasion_to_clarity(final_evasion)
